...
const FREE_FLASH_LITE_TOKEN_LIMIT = 250000;
...
const MAX_FLASH_LITE_TOKEN_LIMIT = 250000; // Give them lite access just in case
...
interface AppState {
  ...
  flashLiteTokenLimit: number;
  flashLiteTokensRemaining: number;
...
export function AppStateProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const loadTokenUsage = async () => {
      ...
      if (usage) {
        setTokenInfo({
          flashLiteUsedTokens: usage.flash_lite_used || 0,
        ...
        setTokenInfo({ flashLiteUsedTokens: 0, flashUsedTokens: 0, proUsedTokens: 0, haikuUsedTokens: 0, date: currentMonth });
      }
    };
  ...
  const getFlashLiteTokenLimit = () => {
    switch (tier) {
        case 'max': return MAX_FLASH_LITE_TOKEN_LIMIT;
        case 'pro': return PRO_FLASH_LITE_TOKEN_LIMIT;
        default: return FREE_FLASH_LITE_TOKEN_LIMIT;
    }
  }

  const flashLiteTokenLimit = getFlashLiteTokenLimit();
  ...
  const consumeTokens = useCallback((amount: number, model: SupportedModel) => {
    ...
    setTokenInfo(prev => {
        ...
        let newFlashLiteUsed = isNewMonth ? 0 : prev.flashLiteUsedTokens;
        ...
        if (model === 'pro' && isPremium) {
            ...
            newFlashLiteUsed += amount;
        } else { // model === 'flash'
            newFlashUsed += amount;
        }
        
        const newInfo: TokenInfo = { flashLiteUsedTokens: newFlashLiteUsed, flashUsedTokens: newFlashUsed, proUsedTokens: newProUsed, haikuUsedTokens: newHaikuUsed, date: currentMonth };
        ...
        if (user) {
          supabase
            ...
            .upsert({
              user_id: user.id,
              month: currentMonth,
              flash_lite_used: newFlashLiteUsed,
  ...
  const flashLiteTokensRemaining = isMounted ? Math.max(0, flashLiteTokenLimit - tokenInfo.flashLiteUsedTokens) : flashLiteTokenLimit;
